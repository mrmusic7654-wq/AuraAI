...
class AppController(private val service: AccessibilityService) {
    ...
    companion object {
        val SUPPORTED_APPS = mapOf(
            "deepseek" to "com.deepseek.chat", "chatgpt" to "com.openai.chatgpt",
            "gemini" to "com.google.android.apps.bard", "whatsapp" to "com.whatsapp",
    ...
    suspend fun execute(app: String, steps: List<AppStep>): String {
        openApp(app); delay(2000)
        var r = ""
        for ((i, s) in steps.withIndex()) {
            try {
                when (s.action) {
                    "tap" -> tap(s.target); "type" -> type(s.target)
                    "swipe" -> swipe(s.target == "up"); "read" -> r = read()
                    "wait" -> delay(s.waitMs)
    ...
    suspend fun sendMessage(app: String, msg: String): String = execute(app, listOf(AppStep("tap","Message"), AppStep("type",msg), AppStep("tap","Send"), AppStep("wait","",5000), AppStep("read")))
    suspend fun debug(app: String, error: String): String = sendMessage(app, "Fix this build error:\n$error")
    
    suspend fun waitForResponse(timeout: Int = 120): String {
        var last = ""; val start = System.currentTimeMillis()
        while (System.currentTimeMillis() - start < timeout * 1000L) {
            delay(3000); val cur = read()
    ...
    suspend fun analyzeScreen(apiKey: String, prompt: String): String {
        return try {
            ...
            model.generateContent(content { text("Screen: ${read().take(3000)}\n\n$prompt") }).text ?: "No response"
        } catch (e: Exception) { "Error: ${e.message}" }
    }
    
    fun getCurrentApp(): String {
        val root = service.rootInActiveWindow ?: return "unknown"
        val pkg = root.packageName?.toString() ?: "unknown"; root.recycle(); return pkg
    }
    ...
    suspend fun waitForApp(pkg: String, timeout: Int = 10): Boolean {
        ...
        while (System.currentTimeMillis() - start < timeout * 1000L) { if (isAppOpen(pkg)) return true; delay(1000) }
        return false
    }
    
    fun read(): String {
        val root = service.rootInActiveWindow ?: return ""
        val t = collect(root); root.recycle(); return t
    }
    
    private fun tap(text: String) {
        val root = service.rootInActiveWindow ?: return
        findNode(root, text)?.let { val r = Rect(); it.getBoundsInScreen(r); it.recycle(); tapAt(r.centerX().toFloat(), r.centerY().toFloat()) }
        root.recycle()
    }
