...
class AutomationRepositoryImpl @Inject constructor(
    ...
    override suspend fun createRule(n: String, d: String?, a: String?, t: String?, tm: String?, ac: List<AgentAction>) = try {
        val e = AutomationRuleEntity(UUID.randomUUID().toString(), n, d, a, t, tm, j.encodeToString(kotlinx.serialization.builtins.ListSerializer(AgentAction.serializer()), ac))
    ...
    private fun AutomationRuleData.toE() = AutomationRuleEntity(id, name, description, triggerApp, triggerText, triggerTime, j.encodeToString(kotlinx.serialization.builtins.ListSerializer(AgentAction.serializer()), actions), isEnabled, createdAt, lastTriggered)
}
